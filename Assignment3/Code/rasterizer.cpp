    ...
    for (int x = x_min; x <= x_max; x++) {
        for (int y = y_min; y <= y_max; y++) {
            if (insideTriangle(x, y, t.v)) {
                ...
                float interpolated_depth = 1.f / (alpha / zs[0] + beta / zs[1] + gamma / zs[2]);


                if (interpolated_depth < depth_buf[get_index(x, y)]) {

                    // Then, we interpolate rest of the parameters.
                    // The color is diffuse color (coeffitio)
                    Vector3f interpolated_color = interpolated_depth *
                                                  (alpha / zs[0] * t.color[0] + beta / zs[1] * t.color[1] +
                                                   gamma / zs[2] * t.color[2]);
                    Vector3f interpolated_normal = interpolated_depth *
                                                   (alpha / zs[0] * t.normal[0] + beta / zs[1] * t.normal[1] +
                                                    gamma / zs[2] * t.normal[2]);
                    Vector2f interpolated_texcoords = interpolated_depth *
                                                      (alpha / zs[0] * t.tex_coords[0] + beta / zs[1] * t.tex_coords[1] +
                                                              gamma / zs[2] * t.tex_coords[2]);
                    // The shading point coordinate in view space (clipping space)
                    Vector3f interpolated_shadingcoords = interpolated_depth *
                                                          (alpha / zs[0] * view_pos[0] + beta / zs[1] * view_pos[1] +
                                                           gamma / zs[2] * view_pos[2]);

                    auto payload = fragment_shader_payload(interpolated_color, interpolated_normal.normalized(),
                                                           interpolated_texcoords, texture ? &*texture : nullptr);
                    payload.view_pos = interpolated_shadingcoords; // Set the shading point coordinate
